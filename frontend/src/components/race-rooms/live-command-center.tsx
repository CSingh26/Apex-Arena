...
function number(value: string, driver: DriverRaceState): number {
...
function rowsFromState(state: RaceState): TimingRow[] {
  return Object.entries(state.drivers)
    .map(([key, driver]) => {
      const driverNumber = number(key, driver);
      const name = driver.full_name ?? driver.broadcast_name ?? `Driver ${driverNumber}`;
      const surname = name.split(" ").at(-1) ?? String(driverNumber);
      const stintStart = typeof driver.stint.lap_start === "number" ? driver.stint.lap_start : typeof driver.stint.start_lap === "number" ? driver.stint.start_lap : null;
      return {
        number: driverNumber,
        name,
        ...
        tyreAge: stintStart != null && state.current_lap != null && state.current_lap >= stintStart ? state.current_lap - stintStart + 1 : null,
...
function locationPoints(state: RaceState, rows: TimingRow[]) {
  const raw = rows.flatMap((row) => {
    ...
    return typeof location?.x === "number" && typeof location.y === "number" ? [{ row, x: location.x, y: location.y }] : [];
  });
  ...
  return raw.map((point) => ({ ...point, x: 12 + ((point.x - minX) / width) * 76, y: 12 + ((point.y - minY) / height) * 76 }));
}

function telemetryMetric(label: string, value: number | boolean | undefined, suffix = "") {
  const display = typeof value === "boolean" ? value ? "ACTIVE" : "OFF" : typeof value === "number" ? `${Math.round(value)}${suffix}` : "—";
  return <div><span>{label}</span><b>{display}</b></div>;
}

export function LiveCommandCenter({ sessionKey, selectedDriver, onSelectDriver }: { sessionKey: string | null; selectedDriver: number | null; onSelectDriver: (driver: number) => void }) {
  ...
  useEffect(() => {
    ...
    getSessionState(sessionKey, controller.signal).then(({ state: initial }) => setNewest(initial)).catch(() => setConnection("unavailable"));
    const connect = () => {
    ...
    connect();
    return () => { controller.abort(); source?.close(); if (timer != null) window.clearTimeout(timer); };
  }, [sessionKey]);
  ...
  const trackStatus = String(state?.race_control_state.event_type ?? (state?.status === "finished" ? "COMPLETED" : "GREEN")).replaceAll("_", " ");
