...
export function PlaybackControls({ room, playback, busy, error, onReplay, onControl }: PlaybackControlsProps) {
  ...
  return <section className="playback-bar" data-testid="playback-controls" aria-label="Replay controls" aria-busy={busy}>
    ...
    <div className="playback-progress" aria-label={`Replay at lap ${playback.current_lap ?? 0} and event ${playback.current_event_sequence}`}>
      <span className="playback-progress__fill" style={{ width: `${room.total_laps && playback.current_lap ? Math.min(100, playback.current_lap / room.total_laps * 100) : 0}%` }} />
    </div>
    <div className="playback-bar__readout" aria-live="polite"><span>Lap <b>{playback.current_lap ?? "—"}</b> / {room.total_laps ?? "—"}</span><span>Event <b>{playback.current_event_sequence}</b></span><span>Messages <b>{playback.current_message_sequence}</b></span><span className={`playback-state playback-state--${playback.is_paused ? "paused" : "running"}`} data-testid="playback-status">{complete ? "Replay complete" : playback.is_paused ? "Paused" : "Running"}</span></div>
    {seekOpen && <div id="replay-seek-controls" className="playback-seek">
      <label><span>Lap</span><input type="number" min="0" max={room.total_laps ?? undefined} value={lapTarget} onChange={(event) => setLapTarget(event.target.value)} /></label><button className="control-button" type="button" disabled={busy || !lapTarget} onClick={() => onControl({ action: "seek_to_lap", lap_number: Number(lapTarget) })}>Go to lap</button>
